...
router.get("/", async(req, res, next) => {
    try {
        ...
        const [countRows] = await db.query(
            `SELECT COUNT(*) AS total FROM flashcards${whereSql}`,
            params
        );
...
router.patch("/:id", async(req, res, next) => {
    try {
        ...
        const { errors, value } = validateFlashcardPayload(req.body, { partial: true });
        if (errors.length) {
            throw createHttpError(400, "Niepoprawne dane wejściowe", errors);
        }

        if (!Object.keys(value).length) {
            throw createHttpError(400, "Brak pól do aktualizacji");
        }

        const [existingRows] = await db.query("SELECT * FROM flashcards WHERE id = ? LIMIT 1", [id]);
        ...
        const merged = {
            ...
            era: value.era ?? existing.era,
            question: value.question ?? existing.question,
            answers: value.answers ?? existing.answers,
            correctAnswer: value.correctAnswer ?? existing.correctAnswer
        };

        if (!merged.answers.includes(merged.correctAnswer)) {
            throw createHttpError(400, "correctAnswer musi istnieć na liście answers");
        }

        await db.query(
            `UPDATE flashcards SET area = ?, era = ?, question = ?, answers = ?, ${correctAnswerColumn} = ? WHERE id = ?`,
            [merged.area, merged.era, merged.question, JSON.stringify(merged.answers), merged.correctAnswer, id]
        );
